    ...
    for code, td in latest_map.items():
        ...
        if reasons:
            out.append({"code": code, "name": name,
                        ...
                        "in_watchlist": code in wl, "mode": "watchlist"})
...
def compute_market_movers(spot: List[dict], top_n: int = 20) -> List[dict]:
    """全市场异动初筛：量比/涨幅/成交额门槛，剔除 ST/北交所/未开盘，取强度 Top N。"""
    c = _cfg()
...
def refresh(conn: sqlite3.Connection, as_of: Optional[str] = None) -> dict:
    ...
    if spot is not None:
        ...
        mode = "watchlist"
    n = dynpool.upsert_pool_rows(conn, "movers", rows, day)
